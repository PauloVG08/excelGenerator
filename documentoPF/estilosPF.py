...
class ClaseEstilos:
    ...
    #Se agrega el color azul cielo a la celda que cuente con este color por default
    def agregarEstiloAzulCielo(self, libro):
        formato = libro.add_format({'bg_color': "#96EFEF", 'border': 1, 'align' : 'center', 'bold' : True})
        formato.set_font_name('Arial')
        formato.set_font_size(10)
        return formato

    #Se agrega el color azul cielo a la celda con informacion que cuente con este color por default
    def agregarEstiloAzulCieloInfo(self, libro):
    ...
    def agregarColumnasEncabezado(self, libro, hoja):
        ...
        #Celdas con color azul cielo
        hoja.write(1, 2, "Identificador de medio", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 4, "Nota otorgante", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 8, "Apellido adicional", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 12, "CURP", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 13, "Numero Seguridad Social", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 15, "Residencia", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 16, "Numero Licencia Conducir", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 17, "Estado civil", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 18, "Sexo", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 19, "Clave Electoral IFE", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 20, "Numero dependientes", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 21, "Fecha de función", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 22, "Indicador de función", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 23, "Tipo persona", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 30, "Fecha residencia", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 31, "Número teléfono", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 32, "Tipo domicilio", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 33, "Tipo asentamiento", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 34, "Origen domicilio", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 42, "Número teléfono", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 43, "Extensión", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 44, "Fax", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 45, "Puesto", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 46, "Fecha contratación", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 47, "Clave moneda", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 48, "Salario mensual", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 49, "Fecha último día de empleo", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 50, "Fecha verificación empleo", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 51, "Origen Razón Social Domicilio", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 59, "Valor activo valuación", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 66, "Fecha cierre cuenta", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 68, "Garantía", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 73, "Número pagos vencidos", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 75, "Histórico pagos", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 76, "Clave prevención", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 77, "Total pagos reportados", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 78, "Clave anterior otorgante", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 79, "Nombre anterior otorgante", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 80, "Número cuenta anterior", self.agregarEstiloAzulCielo(libro))
        hoja.write(1, 90, "Correo electrónico consumidor", self.agregarEstiloAzulCielo(libro))
